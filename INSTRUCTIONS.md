...
```bash
sudo pacman -U cctv-viewer2-2.1.3-1-x86_64.pkg.tar.zst
```
...
* **Allow running multiple instances**: Checking this box allows launching multiple parallel copies of the CCTV Viewer 2 process (by default, it restricts runs to a single active instance).
...
Presents an interactive timeline showing the complete release history, updates, bug fixes, and feature additions of CCTV Viewer 2, ensuring you have direct access to program updates details.
