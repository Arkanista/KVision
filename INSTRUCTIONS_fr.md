...
* {ICON:quick_play} **Lecture rapide (mini-lecteur)** : lance une lecture d'archive dans la fenêtre d'affichage des 30 dernières minutes, vous permettant de revoir rapidement les séquences récentes directement dans la vignette de la caméra sans quitter la vue en direct.
...
Dans le coin inférieur droit de chaque vignette/fenêtre de caméra, un panneau de commande avec cinq icônes fonctionnelles s'affiche lorsque vous passez le curseur de la souris dessus (en fonction des préférences de l'interface utilisateur) :
...
3. **Icône de flèche circulaire (lecture rapide/mini-lecteur)** : active une lecture rapide dans la fenêtre d'affichage des enregistrements d'archives de la caméra des 30 dernières minutes directement à l'intérieur de la vignette de la caméra sans quitter la grille en direct ni lancer la fenêtre d'archive complète.
   * **Panneau de configuration** : lorsqu'il est actif, un panneau de configuration coulissant apparaît en bas de la fenêtre contenant :
     * **Lecture/Pause** : met en pause ou reprend le flux de lecture.
     * **Affichage de l'heure et du décalage** : affiche l'horodatage de lecture en cours et son âge relatif par rapport au temps réel (par exemple, `-12:34` représente il y a 12 minutes et 34 secondes).
     * **Curseur de chronologie interactif** : permet de cliquer ou de faire glisser pour rechercher n'importe où dans la mémoire tampon de 30 minutes. Les segments d'enregistrement disponibles sont mis en évidence sur un fond bleu sarcelle et une tête de lecture rouge marque la position actuelle.
     * **Multiplicateur de vitesse** : passe par les vitesses de lecture (`1x`, `2x` et `4x`) pour numériser rapidement les images.
     * **Fermer (✕)** : ferme le mini-lecteur et ramène instantanément la fenêtre au flux en direct.
   * **Prise en charge du zoom panoramique** : tout comme dans l'affichage en direct, vous pouvez utiliser l'outil loupe pour zoomer sur une région du mini-lecteur, ou maintenir le bouton central de la souris enfoncé et faire glisser pour effectuer un panoramique.
4. **Icône 1:1 (échelle native)** : bascule le mode d'affichage vidéo pixel à pixel. Lorsqu'elle est activée, la vidéo n'est pas étirée ou déformée pour remplir les limites des vignettes, mais est plutôt centrée et affichée dans sa résolution native d'origine. Lorsque ce mode est actif, l'arrière-plan du bouton et le texte « 1:1 » sont mis en évidence en néon turquoise vif.
5. **Icône de loupe (zoom interactif)** : vous permet d'agrandir n'importe quelle région spécifique du flux vidéo :
