...
```bash
sudo pacman -U kvision-2.4.3-1-x86_64.pkg.tar.zst
```
...
* **Domyślne opcje FFmpeg i masowa aktualizacja**: Umożliwia edycję globalnych parametrów FFmpeg dla nowych kamer (zalecane: `-analyzeduration 100000 -probesize 500000 -fflags nobuffer -flags low_delay`).
  * > **[ZALECENIE]** Jeśli używasz starszej konfiguracji KVision (sprzed wersji 2.4.5), zdecydowanie zalecamy zmianę parametru `-analyzeduration 0` na `-analyzeduration 100000` i kliknięcie przycisku "Zaktualizuj wszystkie kamery". Ustawienie wartości "0" na kamerach pozbawionych strumienia audio powoduje opóźnienia przy przełączaniu, gdyż FFmpeg bezskutecznie próbuje "wyszukać" niedostępne pakiety dźwiękowe aż do upłynięcia sztywnego, pięciosekundowego limitu czasowego. Ustawienie bardzo niskiej wartości (np. 100000 mikrosekund = 0.1 sekundy) niweluje ten błąd i zapewnia błyskawiczne wczytywanie strumieni RTSP.
